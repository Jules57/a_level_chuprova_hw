with open('word_file.txt', 'w') as outfile:
    data = """Today's grandparents are joining their grandchildren on social media, but the different generations' online 
...
converted_dict = {k: v for k, v in sorted(word_count.items(), key=lambda item: item[1], reverse=True)}
